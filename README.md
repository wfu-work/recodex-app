...
输入框的「＋」支持相册/拍照（手机）、选择图片（桌面）和粘贴图片；桌面也可将图片拖入输入框。附件可预览、删除，随当前任务的草稿保存，最多 4 张。发送前会将图片缩放至最长边 2048 像素并转换成 PNG，单张上限 6 MB；普通文件、工作区引用和 Skills 选择尚未开放。

需要配套插件在 `host.get_status` 的 `capabilities.imageAttachments.version` 返回 `1`。图片按已认证的 Relay 命令分块上传，Flutter 无需访问电脑端口，Relay 服务无需增加业务 API。上传失败保留草稿，重试续传；发送回执不确定时先核对任务，避免重复执行。草稿目前保存在页面内存中，退出应用不会保存未发送的附件。

本次新增原生图片插件，首次更新需要重新构建 Flutter 应用（仅热重载不足以注册原生插件），并将配套 Codex Relay 插件更新后重启。
